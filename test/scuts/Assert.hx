/**
 * Modified Assert version from franco ponticellis utest library. This version works together with the munit framework of massive.
 */
...
class Assert {
	...
	*/
	public static function isTrue(cond : Bool, ?msg : String, ?pos : PosInfos) {
		...
		if (null == msg)
			msg = "expected true";
		if(!cond)
			massive.munit.Assert.fail(msg, pos);	
	...
	static function sameAs(expected : Dynamic, value : Dynamic, status : LikeStatus) {
		...
		switch(Type.typeof(expected))
		{
			...
			case TClass(c):
				...
				// hash, inthash
				if(Std.is(expected, StringMap) || Std.is(expected, IntMap)) {
					if(status.recursive || status.path == '') {
	...
	*/
	...
	public static function notSame(unexpected : Dynamic, value : Dynamic, ?recursive : Bool, ?msg : String, ?pos : PosInfos) 
	{
		massive.munit.Assert.assertionCount++;
		var status = { recursive : null == recursive ? true : recursive, path : '', error : null };
		if(sameAs(unexpected, value, status)) {
			Assert.fail(msg == null ? value + " is the same as " + unexpected + " although it shouldn't" : msg, pos);
		} else {
			Assert.isTrue(true, msg, pos);
