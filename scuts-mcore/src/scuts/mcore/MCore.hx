...
class MCore
{
  ...
  public static function getFunctionTypeParameters (type:Type, functionName:String):Array<Type>
  {
    function loop (type:Type, found:Array<Type>)
    {
      ...
      {
        return params.foldLeft(found, function (acc, cur) return loop(cur, acc));
      }
      ...
      return switch (type)
      {
        ...
        case TInst(t, p):
          ...
          if (isFunctionTypeParameter(t.get()) && !found.any(Types.eq.bind(_,type)))
        ...
        case TEnum(_, p), TType(_, p):
          searchInParams(p);
        case TFun(args, ret):
          var res = args.foldLeft(found, function (acc, cur) return loop(cur.t, acc));
          loop(ret, res);
        case TAnonymous(a):
  ...
  static var typeCache:Map<String,Array<TypeCacheKey>> = new Map();
	...
	static public function getTypesFromFile (file:String, pack:Array<String>, moduleName:String, ?filter: TypeCacheKey->Bool):Array<TypeCacheKey>
  {
    ...
    return
      ...
      else
      {
        ...
        return matchAll(enumPattern, content, f.bind(TEnum))
        .concat( matchAll(classPattern, content, f.bind(TClass)) )
        .concat( matchAll(typedefPattern, content, f.bind(TTypedef)) )
        .concat( matchAll(interfacePattern, content, f.bind(TInterface)) );
      }
	}
  ...
  {
    // TODO This is a workaround because it seems that haxe.macro.Context.follow throws an error for functions (StdTypes.Int -> hots._)
