...
class CategorysM
{
  macro public static function id_ <A, Cat>(a:ExprOf<A>, catName:ExprOf<String>):ExprOf<Cat<A, A>>
  {
  ...
  macro public static function next_ <A,B,C,Cat>(f:ExprOf<Cat<A, B>>, g:ExprOf<Cat<B, C>>):ExprOf<Cat<A, C>>
  {
  	return R.resolve(macro @:pos(f.pos) scuts.ht.syntax.Categorys.next, [f,g], 3);
  }

  macro public static function dot_ <A,B,C, Cat>(g:ExprOf<Cat<B, C>>, f:ExprOf<Cat<A, B>>):ExprOf<Cat<A, C>>
  {
  	return R.resolve(macro @:pos(g.pos) scuts.ht.syntax.Categorys.dot, [g,f], 3);
  }

  macro public static function back_ <A,B,C, Cat>(g:ExprOf<Cat<B, C>>, f:ExprOf<Cat<A, B>>):ExprOf<Cat<A, C>>
  {
