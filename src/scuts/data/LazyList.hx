...
class LazyListExt {
  ...
  public static function fromArray <T> (arr:Array<T>):LazyList<T> {
    var cp = arr.copy();
  ...
  public static inline function fromIterable <T> (it:Iterable<T>):LazyList<T> {
    return fromIterator(it.iterator());
  }
  
  public static inline function fromIterator <T> (it:Iterator<T>):LazyList<T> {
  ...
  public static function foldLeft <A,B> (list:LazyList<B>, f:A->B->A, init:A):A {
  ...
  public static function foldLeftWithIndex <A,B> (list:LazyList<B>, f:A->B->Int->A, init:A):A {
    var i = 0;
    for (e in toArray(list)) {
      init = f(init, e, i++);
    }
  ...
  public static function toString <T> (list:LazyList<T>, s:Show<T>):String {
    return foldLeftWithIndex(list, function (acc, el, i) return acc + ((i>0) ? ", " : "") + s.show(el), "[") + "]";
  }
  
  public static function fromArrayAsView <T> (arr:Array<T>, index:Int = 0, to:Int = -1):LazyList<T> {
   ...
   */
  public static function last <T> (l:LazyList<T>):T {
  ...
  public static function first <T> (l:LazyList<T>):T {
    return switch (l()) {
  ...
  public static function firstOption <T> (l:LazyList<T>):Option<T> {
    return switch (l()) {
      case LazyNil: None;
  ...
  public static inline function mkEmpty <T> ():LazyList<T> {
    return EMPTY_LIST; 
  }
  
  public static function mkOne <T>(e:T):LazyList<T> {
    return Lazy.expr(LazyCons(e, mkEmpty()));
  }
  
  public static function mkOneThunk <T>(e:Void->T):LazyList<T> {
    #if debug
  ...
  public static function cons <T>(l:LazyList<T>, el:T):LazyList<T> {
  ...
  public static function append <T>(l:LazyList<T>, el:T):LazyList<T> {
  ...
  public static function concat <T>(l1:LazyList<T>, l2:LazyList<T>) {
    return Lazy.expr({
      switch (l1()) {
        case LazyNil: l2();
  ...
  public static function flatMap <S, T>(l:LazyList<S>, f:S->LazyList<T>):LazyList<T>
  {
    return flatten(map(l,f));
  }
  
  public static function flatten <S,T>(l:LazyList<LazyList<T>>):LazyList<T>
  {
    return Lazy.expr({
      switch (l()) {
        case LazyNil:           LazyNil;
