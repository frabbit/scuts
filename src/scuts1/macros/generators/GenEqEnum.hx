...
class GenEqEnum 
{
  ...
  static private function buildClass(data:ClassData):Tup2<String, String>
  {
    ...
    var members = 
      depStrings
      .map(function (x) return "\tvar eq" + x + ":" + "scuts1.classes.Eq<" + x + ">;");
    ...
    var constructArgsStr = 
      depStrings
      .map(function (x) return "eq" + x + ": scuts1.classes.Eq<" + x + ">")
      .join(",");
    
    var instType = instName + classParamsStr;
    var extendsType = "scuts1.classes.EqAbstract<" + instType +  ">";
      ...
      return switch (x) {
        ...
        case Complex(name, _, params): 
          ...
          function createExpr (x:{t:Type, opt:Bool}, i:Int) {
            ...
            return "scuts1.macros.Resolver.tc(" + arg1 + ", scuts1.classes.Eq, [" + memberArgs.join(",") + "]).eq(" + arg1 + "," + arg2 + ")";
          }
