...
abstract Of<M,A>(OfI<M,A>) 
{
	...
	#if !macro
	@:arrayAccess public static inline function arrayGet <A>(x:OfI<Array<In>, A>, index:Int):A return CastHelper.checkType(var _ : Array<A> = cast x)[index];
	...
	@:to public inline static function toArray <A>(x:OfI<Array<In>, A>):Array<A> return cast x;

	@:from public static inline function fromOption (x:Option<A>):Of<Option<In>, A> return new Of(x);
	@:to public static function toOption <A>(x:OfI<Option<In>, A>):Option<A> return CastHelper.preservedCast(cast x);

	@:from public static inline function fromIo (x:Io<A>):Of<Io<In>, A> return new Of(x);
	@:to public static function toIo <A>(x:OfI<Io<In>, A>):Io<A> return CastHelper.preservedCast(cast x);

	@:from public static inline function fromPromise (x:PromiseD<A>):Of<PromiseD<In>, A> return new Of(x);
	@:to public static function toPromise <A>(x:OfI<PromiseD<In>, A>):PromiseD<A> return CastHelper.preservedCast(cast x);

	@:from public static inline function fromList (x:List<A>):Of<List<In>, A> return new Of(x);
	@:to public static function toList <A>(x:OfI<List<In>, A>):List<A> return CastHelper.preservedCast(cast x);

	@:from public static inline function fromCont <A,R>(x:Cont<A,R>):Of<Cont<In,R>, A> return new Of(x);
	@:to public static function toCont <A,R>(x:OfI<Cont<In,R>, A>):Cont<A,R> return CastHelper.preservedCast(cast x);
	...
	@:to public static function toLazy <A>(x:OfI<Void->In, A>):Lazy<A> return CastHelper.preservedCast(cast x);

	@:from public static inline function fromEither <L,R>(x:Either<L,R>):Of<Either<L, In>, R> return CastHelper.preservedCheckType(var _ :  Of<Either<L, In>, R> = new Of(x));
	@:to public static function toEither <L,R>(x:OfI<Either<L, In>, R>):Either<L, R> return CastHelper.preservedCast(cast x);


	@:from public static inline function fromLazyList <T>(x:LazyList<T>):Of<LazyList<In>, T> return new Of(x);
	@:to public static function toLazyList <T>(x:OfI<LazyList<In>, T>):LazyList<T> return CastHelper.preservedCast(cast x);

	@:from public static inline function fromImList <T>(x:ImList<T>):Of<ImList<In>, T> return new Of(x);
	@:to public static function toImList <T>(x:OfI<ImList<In>, T>):ImList<T> return CastHelper.preservedCast(cast x);
