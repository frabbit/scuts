...
class RealResolver
{
  ...
  static var firstCache : StringMap<Expr> = new StringMap();
  static var resolveCache : StringMap<Validation<ResolverError, Expr>> = new StringMap();
  ...
  public static function withCache <X>(cache:StringMap<X>, cacheId:String, f:Expr, args:Array<Expr>, numArgs:Int, mk:Void->X):X {
    var pos = Context.getPosInfos(f.pos);
    var id = pos.min + ":" + pos.max + ":" + ExprTools.toString(f);
    return if (cache.exists(id)) {
      //trace("Cache: " + ExprTools.toString(macro $e($a{args}) ));
      cache.get(id);
    } else {
      //trace("Resolve (" + cacheId + "): " + ExprTools.toString(macro $f($a{args}) ));
      //trace(id);
      var e = mk();
      cache.set(id, e);
      //trace("Resolved (" + cacheId + "):" + ExprTools.toString(macro $f($a{args}) ));
      e;
    }
  }
    ...
    return Profiler.profile(function ()
    {
      return if (isMacroCall(f)) {
        ...
        return Profiler.profile(function () return if (outsourceFirstArg || outsourceFunc)
        {
          ...
          if (numArgs != -1) newArgs.push(macro @:pos(p) @:resolveParamsCount $v{numArgs});
          ...
          blockExprs.push(macro @:pos(p) scuts.ht.core.Ht.resolve($a{newArgs}));
          ...
          //trace("------------- pre regular");
          //var r = withCache(firstCache,"first", f, args, numArgs, resolveRegular);
          var r = resolveRegular();
          //trace("------------- after regular");
          //trace(ExprTools.toString(r));
          return macro @:pos(r.pos) @:allowPrivate $r;
  ...
  static function resolve1 (f:Expr, funtyped:Expr, args:Array<Expr>, scopes:Scopes, lastNeeded:Array<Expr>, numArgs:Int = -1):Validation<ResolverError, Expr>
  {
    ...
    return withCache(resolveCache, "second", f,args,numArgs, function () return
      return Profiler.profile(function ()
      {
        ...
        return switch (numFunctionParams)
        {
          ...
          case Success(numParams):
            ...
            if (!isBind && args.length == numParams) {
              var callExpr = macro @:pos(funtyped.pos) $funtyped($a{args});
              Success(callExpr);
            } else {
  ...
  static inline function  first (e :Expr):Expr {
    return macro @:pos(e.pos) $helper.first($e);
  }
  ...
  {
    return macro @:pos(e.pos) $helper.toImplicitObject($e);
  }
  ...
  static function makeFunctionExprReturnTypeConcrete (f:Expr, returnType:Expr, numArgs:Int)
  {
    ...
    {
      case 0: macro @:pos(f.pos) $helper.typed0($f, $returnType);
      case 1: macro @:pos(f.pos) $helper.typed1($f, $returnType);
      case 2: macro @:pos(f.pos) $helper.typed2($f, $returnType);
      case 3: macro @:pos(f.pos) $helper.typed3($f, $returnType);
      case 4: macro @:pos(f.pos) $helper.typed4($f, $returnType);
      case 5: macro @:pos(f.pos) $helper.typed5($f, $returnType);
      case 6: macro @:pos(f.pos) $helper.typed6($f, $returnType);
  ...
  static function getFunctionExprReturnType (f:Expr, numArgs:Int)
  {
    return switch (numArgs)
    {
      case 0: macro @:pos(f.pos) $helper.ret0($f);
      case 1: macro @:pos(f.pos) $helper.ret1($f);
      case 2: macro @:pos(f.pos) $helper.ret2($f);
      case 3: macro @:pos(f.pos) $helper.ret3($f);
      case 4: macro @:pos(f.pos) $helper.ret4($f);
      case 5: macro @:pos(f.pos) $helper.ret5($f);
      case 6: macro @:pos(f.pos) $helper.ret6($f);
      case _: Scuts.notImplemented();
    }
  }
  ...
  static function resolveImplicitUsingFunction (rawFunc:Expr, required:Expr, requiredType:Option<Type>, args, scopes, lastRequired)
  {
    return Profiler.profile(function ()
    {
      ...
      var isComp = Typer.isCompatible(asImplicitObject(returnedExpr), asImplicitObject(required));
      {
        ...
        //var e = returnedExpr;
        //var to = required;
        //var s1 = ExprTools.toString(e);
        //var s2 = ExprTools.toString(to);
        //var t1 = Typer.typeof(e).map(TypeTools.toString).getOrElseConst("");
        //var t2 = Typer.typeof(to).map(TypeTools.toString).getOrElseConst("");
        //if (t2.indexOf("Promise") > -1 && t2.indexOf("Int") > -1 && t2.indexOf("Show") > -1
        //  && t1.indexOf("Promise") > -1 && t1.indexOf("Show") > -1
        //  )
        //{
        //  //trace(s1);
        //  //trace(s2);
        ...
        //  trace(t1);
        //  trace(t2);
        //  trace(Context.unify(Context.typeof(asImplicitObject(e)), Context.typeof(asImplicitObject(to))));
        //  trace(Context.unify(Context.typeof(e), Context.typeof(to)));
        //  trace(isComp);
        //  trace("---------");
        //}
      }
