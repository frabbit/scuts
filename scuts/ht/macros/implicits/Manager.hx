...
{
  static var ctx = {
    Context.onMacroContextReused(function () { ctx = createContext(); return true;});
    createContext();
  }
   ...
   */
  static function createUniqueVarName () return "__implicit__" + (ctx.uniqueLocalId++);

  /**
   * Registry for local implicit objects.
   */
  //static var localImplicits : Map<String, Array<{e:Expr, pos:{min:Int, max:Int, file:String}}>> = {
  //
  //  new Map();
  //}
  ...
  public static function registerLocals (exprs:Array<Expr>)
  {
    return Profiler.profile(function () {
      ...
      var arr = ctx.localImplicits.get(id);
      ...
      for (e in exprs)
      {
        ...
        var idExpr = macro @:pos(e.pos) $i{id};
        // store local var
  ...
  public static function getLocalImplicits (localContextId:String)
  {
    return Profiler.profile(function () {
      ...
      var all = ctx.localImplicits.get(localContextId);
      if (all == null) return [];
